  ...
  return (
    <section className="question-practice-answer-pane reviewing" aria-label="参考内容与自评">
      ...
      <div className="question-practice-review-scroll" tabIndex={0}>
        ...
        <section><h3>{review.referenceAnswer ? "参考答案" : "答案提纲"}</h3>{review.referenceAnswer ? <p className="question-practice-reference-answer">{review.referenceAnswer}</p> : review.answerOutline.length ? <ol>{review.answerOutline.map((line, index) => <li key={`${index}:${line}`}><span>{index + 1}</span><p>{line}</p></li>)}</ol> : <p className="muted">暂未配置答案提纲。</p>}</section>
