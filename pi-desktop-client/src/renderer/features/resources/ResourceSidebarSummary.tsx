import { AlertTriangle, BrainCircuit, LoaderCircle, PlugZap, RefreshCw } from "lucide-react";
...
export function ResourceSidebarSummary({ view }: { view: Extract<SidebarView, "mcp" | "memory"> }) {
  ...
  const extensionTools = tools.filter((tool) => tool.source.kind === "mcp");

  return (
    <section className="resource-sidebar-summary">
      <header>
        <span className="section-label">{isMcp ? "MCP Runtime" : "Memory Sources"}</span>
        <TooltipIconButton
      ...
      <div className="resource-sidebar-icon">{isMcp ? <PlugZap size={18} /> : <BrainCircuit size={18} />}</div>
      <strong>{isMcp ? `${servers.length} 个 MCP 扩展` : `${memories.length} 个上下文来源`}</strong>
      <p>{isMcp ? `${extensionTools.filter((tool) => tool.active).length}/${extensionTools.length} 个 Tool 已启用` : "内容由 Pi 启动时加载，仅供只读检查。"}</p>
